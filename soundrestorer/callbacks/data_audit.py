# -*- coding: utf-8 -*-
# soundrestorer/callbacks/data_audit.py
...
from soundrestorer.utils.audio import to_mono, normalize_peak, is_silent, match_length
from soundrestorer.utils.metrics import snr_db, si_sdr_db, rms_db
from soundrestorer.utils.io import write_wav, write_csv
...
class DataAuditCallback:
    ...
    Assumptions:
      - batch has "clean" [B,C,T], optionally "noisy".
      - outputs has "yhat" [B,C,T].
    """
    ...
    def on_epoch_begin(self, trainer, epoch: int) -> None:
        ...
        # info line like your current logs
    ...
    def on_train_batch_end(
        ...
        # distribute roughly evenly across batches
        limit = max(1, math.ceil(self.max_items / self.take_from_batches))
        count_here = min(limit, self.max_items - self._saved)
        ...
        for i in range(min(B, count_here)):
            ...
            # align
            y_i, c_i = match_length(y_i, c_i)
            if n_i is not None:
                y_i, n_i = match_length(y_i, n_i)
            ...
            r = dict(
                base=base,
                clean_rms_db=rms_db(c_i),
                yhat_rms_db=rms_db(y_i),
                clean_silence_frac=silence_fraction(c_i),
                yhat_silence_frac=silence_fraction(y_i),
            )
            # quick triad metrics (adds si_yhat_clean_db, snr, delta_si_db if noisy available)
            r.update(triad_metrics(y_i, c_i, n_i))
            if n_i is not None:
                r["noisy_rms_db"] = rms_db(n_i)
                r["noisy_silence_frac"] = silence_fraction(n_i)
    ...
    def on_epoch_end(self, trainer, epoch: int) -> None:
        ...
        # small summary similar to your logs
        snrs = [r.get("snr_noisy_clean_db") for r in self._rows if r.get("snr_noisy_clean_db") is not None]
        nsil = [r.get("noisy_silence_frac") for r in self._rows if r.get("noisy_silence_frac") is not None]
        if snrs:
            mean_snr = sum(snrs)/len(snrs)
            min_snr, max_snr = min(snrs), max(snrs)
        else:
            mean_snr = min_snr = max_snr = float("nan")
        n_sil95 = sum((x or 0.0) > self.silence_threshold for x in nsil)
