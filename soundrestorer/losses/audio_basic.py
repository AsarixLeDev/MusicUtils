...
class PhaseCosine(LossModule):
    def __init__(self, n_fft=1024, hop=256, mag_weight=True):
        self.n_fft = int(n_fft);
        self.hop = int(hop);
        self.mag_weight = bool(mag_weight)
...
class EnergyAnchor(LossModule):
    ...
    def forward(self, outputs, batch):
        y = outputs["yhat"].float();
        x = outputs["noisy"].float()
        y_std = y.std(dim=-1).mean();
        x_std = x.std(dim=-1).mean()
